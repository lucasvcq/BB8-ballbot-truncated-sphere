...
int pwmMaxY = 20;  // Valeur initiale max pour l'avant/arrière en %
  ...
  <head>
    ...
    <style>
      ...
      input[type=range] {
        ...
        appearance: none;
        width: 40px;       /* largeur totale du slider */
        height: 300px;
      ...
      /* Track - barre verticale plus épaisse */
      ...
      input[type=range]::-webkit-slider-thumb {
        -webkit-appearance: none;
        appearance: none;
        width: 28px;            /* diamètre du curseur */
        height: 28px;
        background: white;
        border: 2px solid #333;
        border-radius: 50%;
        margin-left: -10px;     /* recentre horizontalement le pouce sur la barre */
        cursor: pointer;
        position: relative;
        z-index: 1;
      ...
      /* Important : orientation verticale */
      input[type=range] {
        writing-mode: bt-lr;  /* de bas en haut */
...
<body>
  ...
  <script>
    ...
    function updateY(val) {
      ...
      // Récupère l'élément slider
      const ySlider = document.getElementById("ySlider");

      // Calcule la progression entre 0 et 1
      const progress = (val - ySlider.min) / (ySlider.max - ySlider.min);

      // Met à jour le dégradé vertical de la barre
      ySlider.style.background = `linear-gradient(to top, blue ${progress * 100}%, #ddd ${progress * 100}%)`;
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
    if (type == WStype_TEXT) {  // Si le message reçu est un texte (JSON dans notre cas)
        ...
        if (!error) {  // Si le parsing JSON a réussi
            ...
            speedY = map(y, -100, 100, -255*pwmMaxY/100, 255*pwmMaxY/100);  // Limitation de la vitesse max pour éviter d’aller trop vite
