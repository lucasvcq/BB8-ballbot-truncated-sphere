...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
    if (type == WStype_TEXT) {
        ...
        if (!error) {
            int x = doc["x"];  // Contrôle direction (gauche/droite)
            int y = doc["y"];  // Contrôle vitesse (haut/bas)

            // Mapping des vitesses pour moteurs
            speedX = map(x, -100, 100, -255, 255);     // Moteur de rotation
            speedY = map(y, -100, 100, -51, 51);   // Moteur de déplacement
